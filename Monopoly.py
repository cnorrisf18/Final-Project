...
class Player(turtle.Turtle):
    ...
    def __str__(self):
        return self.name
    def addPlayers(self, player1, player2=None, player3=None, player4=None):
        posslist=[player1, player2, player3, player4]
        reallist=[]
        for p in posslist:
    ...
    def goBankrupt(self):
        global tile_list
        properlist = self.getOwned()
        done = False
        while not done:
            sell = input(
                "{}, you're about to go bankrupt! Currently, you've got {} in the bank. Would you like to sell houses "
                "to avoid this? Enter S for sell houses or N for no."
                    .format(self.name, self.bank))
            if sell == 'S' or sell == 's':
                property=None
                propstr = input("What property would you like to sell houses from? Choose from the following:{}".format(self.monohouses))
                if propstr not in properlist:
                    print("Invalid input, try again")
                    continue
                for prop in self.owned:
                    if propstr == prop.name:
                        property = prop
                numhouses = input("How many houses would you like to sell from {}?".format(property.name))
                try:
                ...
                print("{}, you have sold {} houses from {}, which leaves you with {} in the bank.".format(self.name,
                                                                                                          numhouses,
                                                                                                          property.name,
                                                                                                          self.bank))
                if self.bank >= 0:
    ...
    def buyHouses(self, property, numbought):
        ...
        if property.monopoly == False:
            ...
            if property.hashotel is True:
                ...
                if property.name not in self.monohouses:
                    self.monohouses.append(property.name)
            if property.hashotel is True:
    ...
    def sellHouses(self, property, numhouses):
        ...
        if property.howmanyhouses == 0:
            self.monohouses.remove(property.name)
        return self.bank
    def setGo(self):
        global tile_list
    ...
    def rollDice(self):
        ...
        while not donend:
            ...
            if land.canbeowned == True:
                ...
                jailorsafe = unownedProperty(self, land)
                if jailorsafe == 'jail':
                    doned = True
                    return 'wenttojail'
            if dice1 == dice2 and self.bankrupt is False:
...
class Chance(Tiles):
    ...
    def DrawCard(self, player):
        ...
        if self.numcards == 0:
            ...
            chancelist = startchancelist.copy()
        ...
        if drawn == "Advance to Go":
            ...
            for t in tl:
                if t.name == name:
                    ...
                    player.moveTo(go)
            player.collectGo()
        elif drawn =="Advance to Illinois Ave" or drawn == "Advance to St. Charles Place" or\
            ...
            if nownowon.canbeowned is True:
                buyProperty(player, nownowon)
            elif nownowon.canbeowned is False:
                unownedProperty(player, nownowon)
...
class Locations(Tiles):
    ...
    def Sell(self):
        self.owned = False
        self.owner = None
        if self.hashotel is True:
            self.SellHouses(5)
        else:
            self.SellHouses(self.howmanyhouses)
        return self.sellprice
    def Mortgage(self):
        self.mortgaged = True
        return self.mortgageprice
                ...
                if who == other.name:
                    ...
                    if confirm == 'Y' or confirm == 'y':
                        ...
                        if type(sell) == int:
                            ...
                            truelist = []
                            for n in range(0, len(sell.pairs)):
                                if sell.pairs[n] in other.owned:
                                    truelist.append('t')
                                else:
                                    truelist.append('f')
                            if 'f' not in truelist:
                                print(
                                    "{}, you now have a monopoly on the section you just traded for.".format(other.name))
                                other.hasmonopoly = True
                                sell.monopoly = True
                                if sell.islocation is True:
                                    monolist = [sell.name]
                                    for mono in sell.pairs:
                                        mono.monopoly = True
                                        monolist.append(mono.name)
                                    other.monopolies.append(monolist)
                        play.clearstamps()
                        for p in play.owned:
                            play.stampowned(p)
                        other.clearstamps()
                        for o in other.owned:
                            other.stampowned(o)
                        ...
                        if 'f' not in truelist:
                            print(
                                "{}, you now have a monopoly on the section you just bought.".format(play.name))
                            play.hasmonopoly=True
                            buy.monopoly = True
                            if buy.islocation is True:
                                monolist = [buy.name]
...
def monoQuestions(play):
    ...
    if play.hasmonopoly==True:
        ...
        while not done:
            ...
            if buyhouses == 'Y' or buyhouses == 'y':
                ...
                for prop in tl:
                    if prop.name == where:
                        where = prop
                num=input("How many houses would you like to buy? They cost {} each.".format(where.houseprice))
                try:
                    num = int(num)
                except ValueError:
                    print("Invalid input; not an integer")
        ...
        while not end:
            if len(play.monohouses) is not 0:
                ...
                if sellhouses == 'Y' or sellhouses == 'y':
                    where = input('Where would you like to sell houses? Here are your options: {}'.format(play.monohouses))
                    if where not in play.monohouses:
                        print("Invalid input.")
                        continue
                    for prop in tl:
                        if prop.name == where:
                            where = prop
                    num = input("How many houses would you like to sell? They give you {} each.".format(where.houseprice))
                    try:
                        num = int(num)
                    except ValueError:
                        print("Invalid input; not an integer")
...
def givebankamount(playerlist):
    ...
    for n in playerlist:
        bank=n.getBank()
        statement=("{}, your bank balance is {}.".format(n.name, bank))
...
def buyProperty(play, land):
    if land.owned == False:
        ...
        while not done:
            ...
            if buy == 'Y' or buy == 'y':
                complete=play.buyProperty(land)
                if complete == 'success':
                    land.Buy(play)
                    truelist=[]
                    for n in range(0, len(land.pairs)):
                        if land.pairs[n] in play.owned:
                            ...
                            truelist.append('f')
                    if 'f' not in truelist:
                        print("{}, you now have a monopoly on the section you just bought.".format(play.name))
                        land.monopoly = True
                        if land.islocation is True:
                            play.hasmonopoly = True
                            monolist=[land.name]
        ...
        for play in origlist:
            ...
            if play.injail is True:
                ...
                if play.turnsinjail == 3:
                    print("{}, you have spent 3 turns in jail. You've been kicked out!".format(play.name))
...
#the tiles are all constants, along with their lists
#Brown
...
#start chance/chest lists are constants, and the lists need to be outside the class in order to be accessed by multiple objects
...
#the following lists are all constants, used for various functions
...
def mainGame(tl):
    ...
    for p in playerlist:
        plist = playerlist.copy()
        if len(plist)< 4:
            num=4-len(plist)
            for n in range(num):
                plist.append(None)
        p.addPlayers(plist[0], plist[1], plist[2], plist[3])
    banklist=givebankamount(playerlist)
    print('\n'.join(banklist))
    olist=PlayTurn(playerlist)
    winner=findWinner(olist)
    for play in olist:
        if play.bank == winner:
            winner = play
    print("Congrats {}, you have won the game!".format(winner))
    print("These were the ending bank balances:{}".format('\n'.join(givebankamount(olist))))
